import { ISubCategory, ICreateSubCategoryPayload, IUpdateSubCategoryPayload } from "../types/ISubCategoryTypes";
...
const filterFiles = (images: (string | Express.Multer.File)[] | undefined): Express.Multer.File[] => {
  if (!images) return [];
...
const filterStrings = (images: (string | Express.Multer.File)[] | undefined): string[] => {
  if (!images) return [];
